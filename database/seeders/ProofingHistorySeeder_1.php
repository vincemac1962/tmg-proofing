...
class ProofingHistorySeeder_1 extends Seeder
{
    public function run(): void
    {
        $faker = Faker::create();

        // Get jobs older than 4 weeks
        ...
        foreach ($proofingJobs as $job) {
            ...
            Activity::create([
                'job_id' => $job->id,
                'user_id' => $adminUser->id,
                'activity_type' => 'amendment emailed',
                'ip_address' => '127.0.0.1',
                'created_at' => $currentTime,
                'updated_at' => $currentTime,
            ]);

            // Step E: Create approval
            $currentTime = $currentTime->addDay();
            ...
            Approval::create([
                ...
                'customer_id' => $job->customer_id,
                'contract_reference' => $job->contract_reference,
                'approved_by' => $job->customer->user->name,
                'created_at' => $currentTime,
                'updated_at' => $currentTime,
            ]);
