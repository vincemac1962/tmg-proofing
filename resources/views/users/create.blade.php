...
@section('content')
    ...
    <form action="{{ route('users.store') }}" method="POST" onsubmit="return confirm('Are you sure you want to create this user?');">
        @csrf
        ...
        <div class="grid grid-cols-4 gap-4 pt-5 w-1/2">
            ...
            <div class="col-span-2">
                ...
                <select class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 text-gray-900 dark:text-gray-100" id="access_level" name="access_level" required>
                    <option value="0" {{ old('access_level') == '0' ? 'selected' : '' }}>Customer Level Access</option>
                    <option value="1" {{ old('access_level') == '1' ? 'selected' : '' }}>Data Entry & Basic Admin</option>
                    <option value="2" {{ old('access_level') == '2' ? 'selected' : '' }}>Creation & Deletion</option>
                    <option value="3" {{ old('access_level') == '3' ? 'selected' : '' }}>Super Admin</option>
                </select>
            </div>
