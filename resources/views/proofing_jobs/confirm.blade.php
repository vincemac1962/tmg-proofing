...
@section('content')
    ...
    <div class="grid grid-cols-6 gap-4 pt-5 w-full bg-white dark:bg-gray-800 shadow rounded-lg">
        ...
        <div class="md:col-span-2 mb-2">
            ...
            <div id="customer_country" class="col-span-2 w-full px-3 py-2 border border-gray-300 rounded-md dark:bg-gray-700 text-gray-900 dark:text-gray-100">
        ...
        <div class="w-full col-span-6 p-2 items-center text-center">
            <h2 class="text-xl mb-2 block  text-gray-700 dark:text-gray-100 ">Do you want to upload a proof for {{ $customer->company_name }}?</h2>
        </div>
        <!-- fourth row -->
        <div class="col-span-6 flex">
            ...
            <div class="flex-1 flex justify-center items-center">
                <form method="GET" action="{{ route('proofs.create', ['jobId' => $proofingJob->id, 'customerId' => $customer->id]) }}">
                    @csrf
                    <button type="submit" class="text-blue-800 hover:text-blue-600 dark:text-gray-100 dark:hover:text-gray-400">Yes</button>
                </form>
            </div>
        </div>
    </div>
